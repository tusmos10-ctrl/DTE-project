...
func _dash(delta: float) -> void:
	...
	if not hit_this_dash:
		# Distance check
		var dist := global_position.distance_to(player.global_position)
		print("Dashing, dist to player: ", snappedf(dist, 1.0), " hit_radius: ", dash_hit_radius)
		if dist <= dash_hit_radius:
			print("HIT by distance")
			hit_this_dash = true
			_hit_player()
			return
...
func _hit_player() -> void:
	if not is_instance_valid(player):
		return
	# Call die() if the player script has it
	if player.has_method("die"):
		player.die()
		return
	# Fallback built-in death
